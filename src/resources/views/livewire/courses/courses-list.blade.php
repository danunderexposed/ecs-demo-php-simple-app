            ...
            <div class="overflow-hidden bg-white shadow-xl sm:rounded-lg">
                <x-table cols="">
                    <x-slot name="head">
                        ...
                        <x-table.heading>Study Level</x-table.heading>
                        <x-table.heading>Uni / School</x-table.heading>
                        <x-table.heading>Sector</x-table.heading>
                        <x-table.heading>Specialism</x-table.heading>
                    ...
                    <x-slot name="body">
                        @forelse($courses as $c)
                            <x-table.row wire:loading.class.delay="opacity-75 w-full" class="{{ $loop->odd ?: 'bg-gray-50' }}">
                                ...
                                <x-table.cell>
                                    <p class="text-xs">{{ $c->studylevel ? $c->studylevel->studylevel : '' }}</p>
                                </x-table.cell>
                                <x-table.cell>
                                    <p class="text-xs">{{ $c->school->school }}</p>
                                </x-table.cell>
                                <x-table.cell>
                                    <p class="text-xs">{{ $c->sector ? $c->sectorObj->sector : '' }}</p>
                                </x-table.cell>
                                <x-table.cell>
                                    <p class="text-xs">{{ $c->specialism ? $c->specialismObj->specialism : ''}}</p>
                                </x-table.cell>
                                <x-table.cell>
                                    <p class="text-xs">{{ $c->country->name ?? '' }}</p>
                                </x-table.cell>
                                <x-table.cell>
                                    <p class="text-xs">{{ $c->city->name ?? '' }}</p>
                                </x-table.cell>
