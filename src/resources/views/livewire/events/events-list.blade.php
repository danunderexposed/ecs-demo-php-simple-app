            ...
            <div class="overflow-hidden bg-white shadow-xl sm:rounded-lg">
                <x-table cols="">
                    <x-slot name="head">
                        ...
                        <x-table.heading>Display Projects?</x-table.heading>
                        ...
                        @forelse($events as $e)
                            <x-table.row wire:loading.class.delay="opacity-75 w-full" class="{{ $loop->odd ?: 'bg-gray-50' }}">
                                <x-table.cell>
                                    <div class="flex">
                                        @if ($e->image)
                                        <img src="{{ $e->image }}" alt="" class="object-contain w-16 mr-3" >
                                        @endif
                                    </div>
                                </x-table.cell>
                                <x-table.cell>
                                ...
                                <x-table.cell>
                                    <p class="text-xs">{{ $e->displayprojects ? 'Displayed' : 'Not displayed' }}</p>
                                </x-table.cell>
                                <x-table.cell>
                                    <p class="text-xs">{{ $e->featured ? 'Featured' : 'Not featured' }}</p>
                                </x-table.cell>
                                <x-table.cell>
                                    <p class="text-xs">{{ $e->created_at }}</p>
                                </x-table.cell>
