            ...
            <div class="overflow-hidden bg-white shadow-xl sm:rounded-lg">
                <x-table cols="">
                    <x-slot name="head">
                        ...
                        <x-table.heading sortable wire:click="sortBy('homepage.type')" :direction="$sortDirection" sortBy="homepage.type" :currentSortBy="$sortBy">Module Type</x-table.heading>
                        <x-table.heading sortable wire:click="sortBy('homepage.link')" :direction="$sortDirection" sortBy="homepage.link" :currentSortBy="$sortBy">Module Link</x-table.heading>
                        ...
                        @forelse($modules as $m)
                            <x-table.row wire:loading.class.delay="opacity-75 w-full" class="{{ $loop->odd ?: 'bg-gray-50' }}">
                                <x-table.cell>
                                    <div class="flex">
                                        @if ($m->image)
                                        <img src="{{ $m->image }}" alt="" class="object-contain w-16 mr-3" >
                                        @endif
                                        <p>{{ $m->name }}</p>
                                    </div>
                                </x-table.cell>
                                ...
                                <x-table.cell>
                                    <p class="text-sm">{{ $m->link }}</p>
                                </x-table.cell>
                                <x-table.cell class="w-40">
                                    <div class="flex space-x-2"">
                                        <x-button wire:click="edit({{ $m->id }})" primary label="Edit" icon="pencil" />
                                        <x-button
