
<div>
    <x-notifications />
    <x-dialog />
    <x-page-header name="header">
            {{ __('Event Entry Approval') }}
    </x-page-header>

    <div class="flex flex-wrap px-4 pt-6 mx-auto max-w-7xl search xl:px-0">
        <div class="w-full md:w-1/4">
            <x-input wire:model.debounce="search" icon="search" label="" placeholder="Search..." />
        </div>
    </div>
    <div class="py-6 ">
        <div class="px-4 mx-auto max-w-7xl xl:px-0">
            <div class="overflow-hidden bg-white shadow-xl sm:rounded-lg">
                <x-table cols="">
                    <x-slot name="head">
                        <x-table.heading>Image</x-table.heading>
                        <x-table.heading sortable wire:click="sortBy('name')" :direction="$sortDirection" sortBy="name" :currentSortBy="$sortBy">Title</x-table.heading>
                        <x-table.heading>User</x-table.heading>
                        <x-table.heading>Email</x-table.heading>
                        <x-table.heading>Event</x-table.heading>
                        <x-table.heading>School</x-table.heading>
                        <x-table.heading>Entered</x-table.heading>
                        <x-table.heading class="w-40">Actions</x-table.heading>
                    </x-slot>
                    <x-slot name="body">
                        @forelse($entries as $e)
                            <x-table.row wire:loading.class.delay="opacity-75 w-full" class="{{ $loop->odd ?: 'bg-gray-50' }}">
                                <x-table.cell>
                                    <div class="flex">
                                        @if ($e->project && $e->project->cover_large)
                                        <img src="{{ $e->project->cover_large }}" alt="" class="object-contain w-16 mr-3" >
                                        @endif
                                    </div>
                                </x-table.cell>
                                <x-table.cell>
                                    {{ $e->project ? $e->project->title : '' }}
                                </x-table.cell>
                                <x-table.cell>
                                    <p class="text-xs">{{ $e->user ? $e->user->name : '' }}</p>
                                </x-table.cell>
                                <x-table.cell>
                                    <p class="text-xs">{{ $e->user ? $e->user->email : '' }}</p>
                                </x-table.cell>
                                <x-table.cell>
                                    <p class="text-xs">{{ $e->event ? $e->event->name : '' }}</p>
                                </x-table.cell>
                                <x-table.cell>
                                    <p class="text-xs">{{ $e->user ? $e->user->schoolName() : '' }}</p>
                                </x-table.cell>
                                <x-table.cell>
                                    <p class="text-xs">{{ $e->entered }}</p>
                                </x-table.cell>
                                <x-table.cell class="w-40">
                                    <div class="flex space-x-2">
                                        <x-button wire:click="approve({{ $e->id }})" primary label="Approve" icon="check" />
                                        <x-button
                                            wire:click="delete({{ $e->id }})"
                                            label="Remove"
                                            icon="trash"
                                            />
                                    </div>
                                </x-table.cell>
                            </x-table.row>
                        @empty
                            <x-table.row wire:loading.class.delay="opacity-75" class="">
                                <x-table.cell class="py-8 text-center" colspan="9">
                                    {{ __('No entries found')}}
                                </x-table.cell>
                            </x-table.row>
                        @endforelse
                    </x-slot>
                </x-table>
            </div>
        </div>
    </div>
    <div class="pb-6 ">
        <div class="px-4 mx-auto max-w-7xl xl:px-0">
            {{ $entries->links() }}
        </div>
    </div>

</div>
